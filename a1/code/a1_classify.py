...
def recall(C):
    ...
    return np.divide(diag, sum_by_class, out=np.zeros_like(diag), where=sum_by_class!=0)
...
def precision(C):
    ...
    return np.divide(diag, sum_by_classification, out=np.zeros_like(diag), where=sum_by_classification!=0)
